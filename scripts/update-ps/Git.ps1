...
function Invoke-GitPullIfSafe {
  if ($script:SkipGitPull) { Write-Host 'Skipping git pull because -SkipGitPull was supplied.' -ForegroundColor Yellow; return }
  $statusLines = @(Get-GitStatusLines)
  if ($statusLines.Count -gt 0 -and -not $script:AllowDirtyWorktree) {
  ...
  if ($statusLines.Count -gt 0 -and $script:AllowDirtyWorktree) {
...
function Invoke-AutoCommitIfNeeded {
  if ($script:SkipAutoCommit) { Write-Host 'Skipping auto-commit because -SkipAutoCommit was supplied.' -ForegroundColor Yellow; return }
  ...
  Invoke-Native 'git' @('commit', '-m', $script:CommitMessage)
...
function Invoke-AutoPushIfNeeded {
  if ($script:SkipAutoPush) { Write-Host 'Skipping auto-push because -SkipAutoPush was supplied.' -ForegroundColor Yellow; return }
  Set-Location $script:RepoRoot
