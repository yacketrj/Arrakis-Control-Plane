...
export default function AuditTab() {
  ...
  const filtered = useMemo(() => {
    ...
    if (!q) return events
    return events.filter(event => [event.timestamp, event.method, event.path, event.action, event.result, String(event.status)].some(value => value.toLowerCase().includes(q)))
  }, [events, filter])
    ...
    <div className="flex flex-col gap-4 h-full min-h-0">
      <div className="rounded-lg p-4 shrink-0" style={{ background: 'var(--color-surface)', border: '1px solid #2a2418' }}>
        ...
        <p className="text-sm mb-4" style={{ color: 'var(--color-text-dim)' }}>
          Review recent protected mutating admin requests captured by the append-only audit log. The current foundation records endpoint, method, status, duration, and success/failure without storing request bodies or secrets.
        </p>
        <div className="flex gap-3 items-end flex-wrap">
          <div className="flex flex-col gap-1">
            <label className="text-xs" style={{ color: 'var(--color-text-dim)' }}>Filter</label>
            <input className="rounded px-3 py-1.5 text-sm border w-80" style={{ background: 'var(--color-background)', color: 'var(--color-text)', borderColor: '#2a2418', outline: 'none' }} value={filter} onChange={e => setFilter(e.target.value)} placeholder="give-item, teleport, success, failure..." />
          </div>
        ...
        <table className="w-full text-xs">
          <thead>
            <tr style={{ background: '#1a1610', borderBottom: '1px solid #2a2418' }}>
              {['Time', 'Result', 'Method', 'Action', 'Path', 'Status', 'Duration'].map(h => <th key={h} className="text-left px-3 py-2 font-semibold uppercase tracking-wide whitespace-nowrap" style={{ color: 'var(--color-primary)' }}>{h}</th>)}
            </tr>
          </thead>
          <tbody>
            {filtered.map((event, i) => (
              <tr key={`${event.timestamp}-${event.action}-${i}`} style={{ borderBottom: '1px solid #1a1610', background: i % 2 === 0 ? '#0d0b07' : '#0f0d09' }}>
                ...
                <td className="px-3 py-2 font-mono whitespace-nowrap" style={{ color: 'var(--color-text-dim)' }}>{event.path}</td>
            ...
            {filtered.length === 0 && <tr><td colSpan={7} className="px-3 py-8 text-center" style={{ color: 'var(--color-text-dim)' }}>{events.length === 0 ? 'No audit events loaded. Click Refresh.' : 'No events match the filter.'}</td></tr>}
          </tbody>
        </table>
      </div>
    </div>
