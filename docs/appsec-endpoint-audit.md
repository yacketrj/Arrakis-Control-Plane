...
| Mutation reason coverage | In Progress | High-risk/destructive classification, oversized-body reason-enforcement, audit metadata parsing, colorized validation-output changes, and endpoint-by-endpoint audit-event assertion coverage for high-risk/destructive mutations are added pending validation. Pre/post-change review verification still required. |
...
- Audit-event coverage now enumerates high-risk/destructive mutation routes and asserts action, risk, destructive flag, reason/preview flags, status/result, sanitized reason, common target metadata, and request ID pending local validation.
...
| ASEA-003 | High | Partially remediated pending validation | High-risk mutation endpoints require endpoint-by-endpoint verification for `X-Admin-Reason`, audit logging, mutation-safety classification, request-size limits, and pre/post-change review behavior. Initial review found some high-risk mutation paths were under-classified as medium. Follow-up audit-event coverage now enumerates high-risk/destructive mutation routes and asserts action, risk, destructive flag, reason/preview flags, status/result, sanitized reason, common target metadata, and request ID. | Validate the new high-risk/destructive audit-event coverage. Pre/post-change review verification and route-specific target assertions remain open. | Local validation pending. |
