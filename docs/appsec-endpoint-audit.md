...
| Area | Status | Notes |
|---|---|---|
| Route inventory | In Progress | Initial inventory taken from `routes.go`; generated full-route inventory/auth-boundary coverage is validated. |
| Auth boundary review | In Progress | Initial middleware review complete; `appsec_auth_boundary_test.go` covers public, self-service, Discord self-session, representative admin, and WebSocket-ticket boundaries. `appsec_route_inventory_test.go` now parses all registered `routes.go` routes and requires explicit auth-boundary expectations. |
...
| Mutation reason coverage | In Progress | High-risk/destructive classification, oversized-body reason-enforcement, audit metadata parsing, colorized validation-output changes, and endpoint-by-endpoint audit-event assertion coverage for high-risk/destructive mutations are validated. Pre/post-change review verification still required. |
...
- `docs/high-risk-mutation-audit-coverage.md`
...
- Audit-event coverage now enumerates high-risk/destructive mutation routes and asserts action, risk, destructive flag, reason/preview flags, status/result, sanitized reason, common target metadata, and request ID.
...
|---|---|---|---|---|---|
| ASEA-001 | High | Validated partial remediation | No generated endpoint inventory/auth-boundary regression test existed for the full `routes.go` surface. Initial regression coverage existed for public allowlist behavior, self-service path classification, admin-only representative routes, and WebSocket-ticket denial. Generated full-route coverage is now validated. | Keep `appsec_route_inventory_test.go` expectation map current with every `routes.go` change. | `./update.sh` passed clean after generated route inventory/auth-boundary coverage and the test-helper rename fix. |
...
| ASEA-003 | High | Validated partial remediation | High-risk mutation endpoints require endpoint-by-endpoint verification for `X-Admin-Reason`, audit logging, mutation-safety classification, request-size limits, and pre/post-change review behavior. Initial review found some high-risk mutation paths were under-classified as medium. Follow-up audit-event coverage now enumerates high-risk/destructive mutation routes and asserts action, risk, destructive flag, reason/preview flags, status/result, sanitized reason, common target metadata, and request ID. | Keep high-risk/destructive audit-event coverage current with mutation route changes. Pre/post-change review verification and route-specific target assertions remain open. | `./update.sh` passed clean after high-risk/destructive mutation audit-event coverage. |
...
| ASEA-006 | Medium | Validated partial remediation | CORS allowed configured origins and `X-Admin-Token`/`X-Admin-Reason` headers while browser token storage remains JavaScript-readable during the active session. Initial review found allowed-origin parsing should reject unsafe misconfiguration values more strictly. | Hardened allowed-origin parsing to reject wildcard, `null`, control-character, non-HTTP(S), userinfo, path, query, and fragment origins. Added CORS reflection/preflight tests and `docs/browser-token-cors-security.md`. Memory-only or HttpOnly secure session-cookie auth, CSRF design, frontend storage tests, and manual reverse-proxy CORS validation remain open. | `./update.sh` passed clean after CORS/browser-token hardening and tests. |
