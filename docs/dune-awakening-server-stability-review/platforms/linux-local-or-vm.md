...
```text
DuneSandbox process visible:
  Record process ID and launch arguments.

AMP or control panel visible:
  Use the AMP runtime guide.

Docker, containerd, podman, or compose visible:
  Use the Docker/runtime guide.

systemd service visible:
  Use the Linux systemd runtime guide.
```
...
```bash
grep -RniE 'error|warning|LoginRequest|Travel request|ServerState|PreLogin|FinishSpawn|Disconnect|timeout' "$LOG_PATH" 2>/dev/null | head -300
```

## 6. Next Runbooks

Use these after Linux platform discovery:
